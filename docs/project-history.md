...
## 2026-07-07（下午）走查驱动的第二轮大迭代

四路走查（Feedlog 用户反馈 / kimi bridge 真实浏览器对比 4s.link / 代码占位盘点 / 四视角设计评审）后，按 Kane 决策一次性完成：

1. **第一批小修**：线上活动入会链接（onlineUrl）上详情页、拒绝报名补邮件、后台"活动设置"上移、社媒平台本地化（小红书/微博/B站/抖音/公众号）、地图残留绿修复、占位菜单置灰外显（"待上线"标记）。
2. **视觉深度补全**（评审 Top5 处方）：阴影 token 体系（卡片默认态即有厚度）、品牌化图片兜底（钴蓝渐变+首字+点阵母题）、日程页视觉分级（日期跳转条/场地色条/讲者头像/进行中徽章）、钴蓝圆点母题 + 选中态大面积品牌色、地图换肤（钴蓝 pin + 卡片弹窗 + whitesmoke 底图）。
3. **参会者账号体系**：邮箱即账号（验证码登录，无密码），`/me` 我的报名聚合页（状态徽章+电子票）。
4. **统一身份指派**：工作人员=被主办方指派的报名参会者（自己也参加活动），STAFF+APPROVED 自动获得 `/staff/:id/checkin` 核销台权限；嘉宾/VIP/媒体同一指派形态；核销核心抽为 checkin-core 供双端复用。参照 4s.link 实测（Staff 独立管理/Link User 模式/权限与身份分离）。
5. **站内通知**：审核结果/公告/身份指派触发，Header 铃铛未读数，通知中心页（邮件为辅助通道）。
6. **参会者列表**：详情页「谁会来」（三档可见性 + 报名时可选隐藏，公开面只露嘉宾徽章）。
7. **一人一码 + 收藏/分享**：个人通用入场码（邮箱签名，核销台自动匹配本活动报名）、收藏、系统分享 + Worker 注入 OG meta（微信分享出卡片）。
8. **活动后满意度反馈**：主办方问卷编辑（survey-creator）+ 结果/平均分，参会者星级+动态表单（限已通过且活动结束/已签到）。
9. **真双语 i18n**：258 key 双字典、EN/中 切换、localStorage 持久化；日期格式化暂保持中文。

**新增生产资产**：D1 表 email_otps / notifications / bookmarks / feedback_responses（均已建）；无新 secrets（复用 TICKET_SECRET 签名参会者 session 与个人码）。

**已知限制**：OTP 邮件依赖 Resend 验证域名（同既有邮件限制）；活动结束自动邀约反馈未做（无定时任务基建，入口在 /me 手动）；4s.link 对比中判定低优先的 Grid 视图/价格筛选/付费票列入 backlog。
